...
definitionLocation[uri_, src_] /; StringQ[uri] && validDefinitionSourceQ[src] :=
	Module[{adjustedSource},
		...
		<|
			...
			"range" -> <|
				"start" -> <|
					"line"      -> adjustedSource[[1, 1]],
					"character" -> adjustedSource[[1, 2]]
				|>,
				"end"   -> <|
					"line"      -> adjustedSource[[2, 1]],
					"character" -> adjustedSource[[2, 2]]
				|>
			|>
		|>
	]
	...
	Module[
		...
		*)
		If[
			...
			symContext =
				StringJoin[
					Riffle[Most[StringSplit[symbolName, "`"]], "`"],
					"`"
				],
		...
		*)
		...
		If[!StringQ[pacletDir] || !DirectoryQ[pacletDir], Return[{}]];
		If[$Debug2,
			log[
				"findExternalSymbolDefinition: searching in ",
				pacletDir,
				" for ",
				bareSymbol
			]
		...
		*)
		...
		If[Length[kernelFiles] == 0, Return[{}]];
		locations = {};
		(*
		...
		*)
		Do[
			...
			*)
			If[!StringContainsQ[text, bareSymbol], Continue[]];
			(*
			Parse and search for definitions
			*)
			cst = Quiet[CodeConcreteParse[text]];
			If[FailureQ[cst], Continue[]];
			ast =
				Quiet[
				...
				Cases[
					...
					node_[
						...
						AnyTrue[
							...
							MatchQ[
								#,
								LeafNode[Symbol, s_String, _] /;
									StringMatchQ[
										s,
										(__ ~~ "`" ~~ bareSymbol) | bareSymbol
									]
							]&
						] :> src,
					Infinity
			...
			*)
			...
			If[Length[defs] > 0,
				uri = StringJoin[ "file://", file];
				locations =
					Join[
						locations,
						DeleteCases[
							definitionLocation[uri, #]& /@ defs,
							Nothing
						]
					]
			],
...
expandContent[
	content : KeyValuePattern["method" -> "textDocument/definition"],
	pos_
] :=
	Catch[
		Module[{params, id, doc, uri, res},
			...
			uri = doc["uri"];
			If[isStale[$PreExpandContentQueue[[pos[[1]] + 1;;All]], uri],
				log[2, "stale"];
				Throw[
					{
						<|
...
handleContent[
	content : KeyValuePattern["method" -> "textDocument/definitionFencepost"]
] :=
	Catch[
		Module[{
				id,
			...
			*)
			cases =
				Cases[
					...
					LeafNode[
						...
						_,
						KeyValuePattern[
							Source -> src_ /; SourceMemberQ[src, {line, char}]
						]
					],
					Infinity
				];
			...
			preferredContext =
				If[StringContainsQ[rawSymbolName, "`"],
					StringJoin[
						Riffle[Most[StringSplit[rawSymbolName, "`"]], "`"],
						"`"
					],
					None
				];
			...
			*)
			...
			localLocations =
				DeleteCases[definitionLocation[uri, #]& /@ srcs, Nothing];
			(*
			...
			*)
			...
			If[
				...
				*)
				pacletDefinitions =
					LSPServer`PacletIndex`GetVisibleSymbolDefinitions[
						uri,
						symbolName,
						preferredContext
					];
				If[$Debug2,
					log[
						"paclet definitions for ",
						symbolName,
						": ",
						Length[pacletDefinitions]
					]
				];
				(*
			...
			*)
			If[Length[locations] == 0,
				Module[{externalLocations},
					...
					externalLocations =
						findExternalSymbolDefinition[symbolName];
					If[Length[externalLocations] > 0,
						If[$Debug2,
							log[
								"found ",
								Length[externalLocations],
			...
			*)
			...
			If[$Debug2, log["definition locations: ", Length[locations]]];
