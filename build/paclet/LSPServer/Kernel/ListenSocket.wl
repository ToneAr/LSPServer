...
processData[dataByteArray_, sourceSocket_] :=
		...
		parsed = findMessageParts[dataString];
		msgBodies = Lookup[parsed, "lspMsgs", {}];
		If[msgBodies =!= {},
			contentsIn = Developer`ReadRawJSONString /@ msgBodies;
			lspMsgAssoc["lspMsgs"] = {};
			lspMsgAssoc["lspMsg"] = "";
			...
			If[!LSPServer`Private`contentQueueEmptyQ[],
				...
				LSPServer`$WriteLSPResultThunk =
					Function[{c}, writeLSPResult["Socket", sourceSocket, c]];
				While[
					LSPServer`$ContentQueue =!= {},
					content = LSPServer`Private`takeFirstContentQueueItem[];
					log[
						2,
						"taking first from $ContentQueue: ",
						#["method"]&[content]
					];
					log[
						2,
						"rest of $ContentQueue (up to 20): ",
						Take[#["method"]& /@ LSPServer`$ContentQueue, UpTo[20]]
					];
					log[2, "..."];
...
readEvalWriteLoop["ListenSocket", sock_] :=
	SocketListen[
		sock,
		processData[#"DataByteArray", #"SourceSocket"]&,
