...
cstToJSON[GroupNode[kind_, children_, meta_]] :=
	<|
		...
		"source"   -> meta[Source]
	|>
cstToJSON[other_] := <|"type" -> "Unknown", "wl" -> ToString[other, InputForm]|>
...
handleContent[content : KeyValuePattern["method" -> "wolfram/cst"]] :=
	Catch[
		Module[{
			...
			params =
				Replace[
					Lookup[content, "params", <||>],
					Except[_Association] -> <||>
				];
			textDocument =
				Replace[
					Lookup[params, "textDocument", <||>],
					Except[_Association] -> <||>
				];
					...
					Throw[
						{
							<|
								...
								"error"   -> <|
									"code"    -> -32602,
									"message" -> StringJoin[
										"wolfram/cst requires ",
										"textDocument.uri or source"
									]
								|>
							|>
						}
