...
Building with the latest Wolfram version used in CI (currently 14.1) is only guaranteed to work back to Wolfram 14.1+.
...
This table shows the historical value of -mmacosx-version-min for libraries generated by CCompilerDriver through Wolfram 13.3:
...
Later Wolfram versions should be verified against the current compiler toolchain. LSPServer sets the same macosx-version-min in order to achieve maximum compatibility with libraries generated by CCompilerDriver.
